...
SiteTagEntry.prototype.handleOnAvailable = function () {
  ...
  this.main_container.insertBefore(
    this.array_element,
  ...
  this.auto_complete.containerExpandEvent.subscribe(
    function () {
      SwatZIndexManager.raiseElement(this.input_element.parentNode);
    },
    this,
  ...
  for (var i = 0; i < this.initial_selected_tag_array.length; i++) {
    this.addTag(
      this.initial_selected_tag_array[i][1],
  ...
  this.auto_complete.itemArrowToEvent.subscribe(this.itemSelected, this, true);
  ...
  YAHOO.util.Event.on(
    ...
    function (e) {
      YAHOO.lang.later(100, this, function () {
        if (this.auto_complete._sInitInputValue !== this.input_element.value) {
...
SiteTagEntry.prototype.updateAddTagElement = function () {
  if (this.canAddTag(this.input_element.value)) {
    YAHOO.util.Dom.removeClass(this.a_tag, 'site-tag-entry-add-insensitive');
  } else {
  ...
  YAHOO.util.Event.addListener(
    ...
    function (e, entry) {
      ...
      if (
        ...
        var tag_name = entry.input_element.value.slice(0, delimeter_pos);
...
SiteTagEntry.prototype.canAddTag = function (tag_name) {
  var total_tags = this.selected_tag_array.length + this.new_tag_array.length;
...
SiteTagEntry.prototype.addTag = function (tag_name, tag_title) {
  ...
  if (this.maximum_tags > 0 && total_tags >= this.maximum_tags) return;
  ...
  if (new_tag && !this.allow_adding_tags) return;
  ...
  if (this.array_element.lastChild) {
    YAHOO.util.Dom.removeClass(this.array_element.lastChild, 'site-tag-last');
  }
...
SiteTagEntry.prototype.getTagElement = function (tag_name, title, new_tag) {
  ...
  anchor_tag.appendChild(document.createTextNode(SiteTagEntry.remove_text));
...
SiteTagEntry.prototype.removeTag = function (tag_name) {
  ...
  var anchor_tag = document.getElementById(this.id + '_tag_remove_' + tag_name);
  ...
  if (li_tag) {
    ...
    var out_animation = new YAHOO.util.Anim(
      li_tag,
      out_attributes,
      0.25,
    ...
    out_animation.onComplete.subscribe(
      function (e) {
...
SiteTagEntry.prototype.updateVisibility = function () {
  ...
  else YAHOO.util.Dom.removeClass(this.input_element.parentNode, 'swat-hidden');
};
