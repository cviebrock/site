...
function SiteJwPlayerMediaDisplay(media_id, container_id) {
  ...
  this.end_point_recorded_event = new YAHOO.util.CustomEvent(
    'end_point_recorded',
    this,
    true
  );
...
SiteJwPlayerMediaDisplay.restart_video_text = 'Start From the Beginning';
...
SiteJwPlayerMediaDisplay.prototype.embedPlayer = function () {
  ...
  function checkIfCurrent() {
    ...
    for (var i = 0; i < all_players.length; i++) {
      if (
        all_players[i].player_id != SiteJwPlayerMediaDisplay.current_player_id
      ) {
        all_players[i].pause();
      }
    }
...
SiteJwPlayerMediaDisplay.prototype.addImage = function (image_uri, width) {
...
SiteJwPlayerMediaDisplay.prototype.recordEndPoint = function () {
  ...
  function recordEndPoint() {
    ...
    client.callProcedure(
      'recordEndPoint',
      callback,
      [that.media_id, current_position],
      ['int', 'double']
    );
...
SiteJwPlayerMediaDisplay.prototype.getPlayerHeight = function () {
  ...
  return parseInt(
    (region.width * this.aspect_ratio[1]) / this.aspect_ratio[0],
    10
  );
};
...
SiteJwPlayerMediaDisplay.prototype.debug = function () {
  ...
  this.player.on('meta', function (v) {
    ...
    if (meta && meta.hasOwnProperty('transitioning')) {
      content += '<br />transitioning: ' + (meta.transitioning ? 'yes' : 'no');
    }
    ...
    content +=
      '<br />quality-level: <strong>' +
      current_level.label +
      '</strong> (' +
      that.player.getCurrentQuality() +
      ')';

    if (meta && meta.hasOwnProperty('bandwidth')) {
      content +=
        '<br />bandwidth: ' +
        Math.round(meta.bandwidth / 1024, 2) +
        ' Mb/s ' +
        '(' +
        meta.bandwidth +
        ')';
...
SiteJwPlayerMediaDisplay.prototype.drawDialogs = function () {
  ...
  if (this.on_complete_message !== null) {
    ...
    if (
      this.display_on_complete_message_on_load &&
      this.start_position > 0 &&
...
SiteJwPlayerMediaDisplay.prototype.positionOverlay = function (overlay) {
  ...
  if (overlay_region) {
    ...
    var margin = Math.floor(
      Math.max(0, (overlay_region.height - content_region.height) / 2)
    );
